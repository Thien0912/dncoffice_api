...
class Trashvanbanden extends REST_INSTANCE_Controller
{
    ...
    public function index_get()
    {
        ...
        $data = $this->E_van_ban_model->getTrashVanbanden($start, $length, $searchValue, $orderBy, $searchKey, $fromDate, $toDate);
    ...
    public function restore_post()
    {
        ...
        $this->db->trans_start();
        $vanban = $this->E_van_ban_model->where('loai_van_ban', $this->common::VAN_BAN_DEN)->whereIn('id_van_ban', $ids)->where('deleted_at IS NOT NULL')->get();
        if (count($ids) != count($vanban)) {
            resError('Có lỗi xảy ra'); // truyền id không tương ứng với danh sách
        }

        // dd($this->E_van_ban_model->whereIn('id_van_ban', $ids)->get());

        $this->E_van_ban_model->whereIn('id_van_ban', $ids)->update([
            'deleted_at' => null
        ]);
        $this->createLog('restore', 'Khôi phục văn bản đến', $vanban, null, 'e_van_ban');
    ...
    public function force_delete_post()
    {
        ...
        $this->createLog('delete', 'Xóa vĩnh viễn văn bản đến', $vanban,  null, 'e_van_ban');
        $this->db->trans_commit();
