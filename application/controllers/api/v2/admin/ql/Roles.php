...
class Roles extends REST_INSTANCE_Controller
{
    ...
    {
        $roles = $this->Ql_vai_tro_model->get();
        
        $data = array_map(function($role) {
            return [
                'ql_vai_tro_id' => $role['ql_vai_tro_id'],
    ...
    public function show_get($roleId)
    {
        ...
        $userIds = array_unique(array_column($roleUsers, 'ql_nguoi_dung_id'));
        // dd($userIds);
        ...
        $this->response([
            ...
            'success' => true,
            'data' => $role,
            'usersInRole' => !empty($userIds) ? $this->Ql_nguoi_dung_model
                ->join('e_don_vi', 'e_don_vi.id_don_vi = ql_nguoi_dung.id_don_vi', 'left')
                ->whereIn('ql_nguoi_dung.ql_nguoi_dung_id', $userIds)
                ->where('ql_nguoi_dung.active_flag', 1)
                ->select('ql_nguoi_dung.ql_nguoi_dung_id, ql_nguoi_dung.ql_nguoi_dung_ho_ten, ql_nguoi_dung.ql_nguoi_dung_email, ql_nguoi_dung.ql_nguoi_dung_avatar, e_don_vi.*')
                ->get() : [],
            'usersNotInRole' => !empty($userIds) ? $this->Ql_nguoi_dung_model
                ->join('e_don_vi', 'e_don_vi.id_don_vi = ql_nguoi_dung.id_don_vi', 'left')
                ->whereNotIn('ql_nguoi_dung.ql_nguoi_dung_id', $userIds)
