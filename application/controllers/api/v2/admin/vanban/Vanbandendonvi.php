...
class Vanbandendonvi extends REST_INSTANCE_Controller
{
    public function __construct()
    {
        ...
        if (!$this->inSegment([
            ...
            'vanbandendonvi.bao_cao_phan_hoi_create'
        ])) {
            $this->permissionMiddleware();
        }
    ...
    public function index_get()
    {
        ...
        $response = $this->E_van_ban_model->getAllBaoCaoPhanHoiTheoDonvi($data['start'], $data['length'], $data['searchValue'], $data['order'], $data['columns'], $data['searchKey'],  $data['fromDate'], $data['toDate'], $auth, $dataSource);
    ...
    {
        $auth = $this->getUserLogin();
        $donvi = $this->E_don_vi_xu_ly_model->getUnitIdsByDocId($id);
        if (!in_array($auth['id_don_vi'], $donvi)) resError('Bạn không có quyền xem văn bản này');
    ...
    {
        $ids = commonRequest('ids');
        if (empty($ids)) resError('Không có văn bản nào được chọn');

        $user = $this->getUserLogin();
        $this->db->trans_start();

        //Lấy danh sách văn bản theo id
        $vanbans = $this->E_van_ban_model
            ->whereIn('id_van_ban', $ids)
            ->get();

        if (empty($vanbans)) resSuccess(null, 'Không tìm thấy văn bản nào để chuyển vào thùng rác');

        $dataInsert = [];
        $idsValid = [];
        foreach ($vanbans as $vb) {
            //Kiểm tra xem đã có trong thùng rác chưa
            $exist = $this->db
        ...
        resSuccess($idsValid, 'Chuyển vào thùng rác thành công');
    }

    public function save_internal_post($id)
    {
        $user = $this->getUserLogin();
        $this->db->trans_start();

        // 1. Get original doc
        $original = $this->E_van_ban_model->find($id);
        if (!$original) {
            resError('Văn bản không tồn tại', REST_Controller::HTTP_NOT_FOUND);
        }

        // 2. Clone it for internal
        $dataNew = $original;
        unset($dataNew['id_van_ban']);
        unset($dataNew['ngay_tao']);
        unset($dataNew['ngay_sua']);
        $dataNew['nam'] = date('Y');
        $dataNew['loai_van_ban'] = $this->common::VAN_BAN_NOI_BO;
        $dataNew['trang_thai'] = 1;
        $dataNew['id_nguoi_tao'] = $user['ql_nguoi_dung_id'];
        $dataNew['id_nguoi_sua'] = $user['ql_nguoi_dung_id'];
        $dataNew['id_don_vi_soan'] = $user['id_don_vi'];
        $dataNew['id_don_vi'] = $user['id_don_vi'];
        
        $newVb = $this->E_van_ban_model->create($dataNew);
        $newId = $newVb['id_van_ban'];

        // 3. Set viewers: Current user and all leaders of their unit
        ...
        $leaders = $this->db->where('id_don_vi', $user['id_don_vi'])
                            ->where('ql_nguoi_dung_la_lanh_dao', 1)
                            ->get('ql_nguoi_dung')->result_array();
                            
        foreach($leaders as $leader) {
            if($leader['ql_nguoi_dung_id'] != $user['ql_nguoi_dung_id']) {
                $this->db->insert('e_nguoi_xem_vbnoibo', [
                    'id_van_ban' => $newId,
                    'ql_nguoi_dung_id' => $leader['ql_nguoi_dung_id'],
                    'nguoi_dong_so_huu_id' => null
                ]);
            }
        }
        ...
        // 4. Duplicate files
        $files = $this->E_file_dinh_kem_model->where('id_van_ban', $id)->get();
        foreach($files as $file) {
            $dataF = $file;
            unset($dataF['id_file_dinh_kem']);
            $dataF['id_van_ban'] = $newId;
            $this->E_file_dinh_kem_model->create($dataF);
        }

        // 5. Write log
        $this->createLog('create', 'Lưu vào công văn nội bộ từ văn bản đến đơn vị', null, $newVb, 'e_van_ban');
        
        $this->db->trans_commit();

        // Trich xuat index de search fulltext
        call_n8n_trich_xuat($newId);

        resSuccess($newVb, 'Đã lưu vào công văn nội bộ');
    }
    ...
    public function tao_butphe_post($id)
    {
        ...
        $this->createLog('but_phe', 'Bút phê văn bản đến', [], $butphe, 'e_van_ban, e_but_phe');
    ...
    public function update_butphe_put($id)
    {
        ...
        $this->createLog('but_phe', 'Bút phê văn bản đến', $vb['but_phe'], $newVb['but_phe'], 'e_van_ban, e_but_phe');
    ...
    public function xuly_vanban_post($id)
    {
        ...
        $donviChinhNames = !empty($donviChinhIds) ? array_column($this->E_don_vi_model->whereIn('id_don_vi', $donviChinhIds)->get(), 'ten_don_vi') : [];
        $donviPhoihopNames = !empty($donviPhoihopIds) ? array_column($this->E_don_vi_model->whereIn('id_don_vi', $donviPhoihopIds)->get(), 'ten_don_vi') : [];

        $logDetail = [
            'id_van_ban' => $id,
            'don_vi_nhan' => array_merge($donviChinhNames, $donviPhoihopNames),
            'don_vi_xu_ly_chinh' => $donviChinhIds ?? [],
            'don_vi_phoi_hop' => $donviPhoihopIds ?? [],
            'trang_thai_moi' => 'CHO_XU_LY'
        ];

        $this->createLog('chuyen_xu_ly', 'Phân công xử lý văn bản: ' . $vb['so_hieu_van_ban'], [], $logDetail, 'e_xu_ly, e_don_vi_xu_ly');
    ...
    public function change_status_put($id)
    {
        ...
        $newVb = $this->E_van_ban_model->find($id);
        $this->createLog('update_status', 'Cập nhật trạng thái văn bản', $vb, $newVb, 'e_van_ban');
    ...
    public function xem_van_ban_post()
    {
        ...
        try {
            foreach ($ids as $key => $id) {
                ...
                // Kiểm tra văn bản đã bị thu hồi chưa
                if (!empty($vanban['deleted_at'])) {
                    throw new Exception("Văn bản số " . ($vanban['so_hieu_van_ban'] ?? $id) . " đã bị thu hồi, không thể xem nội dung");
                }
                ...
                $Listdvxl = $this->E_don_vi_xu_ly_model
                    ...
                    ->where('id_nguoi_xu_ly', $auth['ql_nguoi_dung_id'])
                    ->get();
                if (empty($Listdvxl) && !$auth['ql_nguoi_dung_is_admin'])  throw new Exception("Bạn không có quyền xem văn bản này");
                ...
                if (!empty($dsThongbao)) {
                    foreach ($dsThongbao as $item) {
                        ...
                        if ($thongBaoNguoiDung  && $thongBaoNguoiDung['ql_thong_bao_da_doc'] == 0) {
                            $this->db->where('ql_thong_bao_nguoi_dung_id', $thongBaoNguoiDung['ql_thong_bao_nguoi_dung_id'])
    ...
    public function files_v2_post()
    {
        ...
        $files = $this->E_van_ban_model->getFiles_v2(null, $offset, $limit, $filters);
    ...
    public function creators_get()
    {
        $creators = $this->E_van_ban_model->getDistinctCreators();
        resSuccess($creators);
    }
}
