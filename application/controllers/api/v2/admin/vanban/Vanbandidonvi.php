...
class Vanbandidonvi extends REST_INSTANCE_Controller
{
    ...
    public function index_get()
    {
        ...
        $current_year_vbddv = commonRequest('current_year_vbdi') ? commonRequest('current_year_vbdi') : null;
        if ($id_loai_selected) {
            $result = $this->soHieuVanBan($auth, 2, $id_loai_selected, $current_year_vbddv);
        ...
        $response = $this->E_van_ban_model->getAllVanbandidonvi($data['start'], $data['length'], $data['searchValue'], $data['order'], $data['columns'], $data['searchKey'],  $data['fromDate'], $data['toDate'], $auth, $dataSource);
        ...
        $data = $this->E_van_ban_model->getAllVanbandidonvi($start, $length, $searchValue, $orderBy, $searchKey, $fromDate, $toDate);
    ...
    public function create_post()
    {
        ...
        if ($soHieuVB) {
            $current_year_vbddv = commonRequest('current_year_vbddv') ? commonRequest('current_year_vbddv') : null;

            $existSoHieu = $this->E_van_ban_model
                ->where('loai_van_ban', 2)
                ->where('so_hieu_van_ban', $soHieuVB)
                ->where('YEAR(ngay_ky)', $current_year_vbddv)
        ...
        $data = [
            ...
            'trang_thai' => $this->common::STATUS_VAN_BAN_DI_DON_VI['DA_BAN_HANH']['value'],
        ...
        if ($ids_don_vi_xu_ly) {
            $this->E_van_ban_model
                ...
                ->update([
                    'trang_thai' => Common::STATUS_VAN_BAN_DI_DON_VI['CHO_XU_LY']['value']
                ]);
        ...
        if ($trang_thai_luu_tru == 'LUU_TRU') {
            $this->E_van_ban_model
                ->where('id_van_ban', $vb['id_van_ban'])
                ->update([
                    'trang_thai' => Common::STATUS_VAN_BAN_DI_DON_VI['LUU_TRU']['value']
                ]);
        } else {
        ...
        $this->createLog('Create', 'Thêm văn bản đi đơn vị: ' . $vb['so_hieu_van_ban'], NULL, $vb, 'e_van_ban');
    ...
    public function show_get($id)
    {
        ...
        $vb = $this->E_van_ban_model->detail_vanbandidonvi($id, $auth);
    ...
    public function update_post($id)
    {
        ...
        // if ($this->checkExitsUpdate($id, 'so_van_ban', $data['so_van_ban'])) {
        //     resError('Trùng lặp số văn bản');
        // }
        ...
        $ids_hinh_thuc_old = array_column(($this->E_vb_hinh_thuc_model->select('*')->where('id_van_ban', $id)->get()), 'id_hinh_thuc');
        $ids_hinh_thuc_old_array = array_unique($ids_hinh_thuc_old);
        ...
        if (($ids_don_vi_xu_ly_array || $ids_ql_nguoi_dung_array) && ($ban_hanh_lai == "true")) {
            $this->createNotification($auth, $vb['id_van_ban'], $ids_don_vi_xu_ly_array, $ids_ql_nguoi_dung_array, false, false);
            // send_zalo_oa_message($vb['id_van_ban']);
        ...
        //Ghi log
        $this->createLog('update', 'Cập nhật văn bản đi: ' . $newVb['so_hieu_van_ban'], $vb, $newVb, 'e_van_ban');
    ...
    public function export_get()
    {
        ...
        $data = $this->E_van_ban_model->getListExportVanbandi($start, $length, $searchValue, $searchKey, $fromDate, $toDate, $this->getUserLogin());
    ...
    //                         $errorMessages = [];
    //                         $field_PrimaryKey = [
    //                             'so_van_ban' => 'Số văn bản đang rỗng.',
    //                             'ten_loai' => 'Tên loại đang rỗng.',
    //                             'trich_yeu' => 'Trích yếu đang rỗng.',
    //                             'nguoi_ky' => 'Người ký đang rỗng.',
    //                             'ten_don_vi_soan' => 'Đơn vị soạn đang rỗng.',
    //                             'ten_hinh_thuc' => 'Hình thức đang rỗng.',
    //                             // 'nguoi_soan_vb_di' => 'Người soạn đang rỗng.',
    //                             'don_vi' => 'Đơn vị đang rỗng.',
    //                             'noi_luu_tru' => 'Nơi lưu trữ đang rỗng.',
    //                             'ten_tinh_chat' => 'Tính chất đang rỗng.',
    //                             'ten_bao_mat' => 'Bảo mật đang rỗng.',
    //                         ];
    ...
    //                         } else {
    //                             $invalidData = false;
    //                             $id_loai = $this->E_loai_model->where('ten_loai', trim($l['ten_loai']))->first();
    //                             $id_don_vi_soan = $this->E_don_vi_model->where('ten_don_vi', trim($l['ten_don_vi_soan']))->first();
    //                             $id_hinh_thuc = $this->E_hinh_thuc_model->where('ten_hinh_thuc', trim($l['ten_hinh_thuc']))->first();
    //                             $nguoi_soan_vb_di = $this->Ql_nguoi_dung_model->where('ql_nguoi_dung_ho_ten', trim($l['nguoi_soan_vb_di']))->first();
    //                             $id_don_vi = $this->E_don_vi_model->where('ten_don_vi', trim($l['don_vi']))->first();
    //                             $id_tinh_chat = $this->E_tinh_chat_model->where('ten_tinh_chat', trim($l['ten_tinh_chat']))->first();
    //                             $id_bao_mat = $this->E_bao_mat_model->where('ten_bao_mat', trim($l['ten_bao_mat']))->first();


    //                             if (!$id_loai) {
    //                                 $l['ketqua'] .= "Loại văn bản này không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('ten_loai', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                             if (!$id_don_vi_soan) {
    //                                 $l['ketqua'] .= "Tên đơn vị soạn không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('ten_don_vi_soan', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                             if (!$id_hinh_thuc) {
    //                                 $l['ketqua'] .= "Tên hình thức không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('ten_hinh_thuc', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                             // if (!$nguoi_soan_vb_di) {
    //                             //     $l['ketqua'] .= "Tên người soạn văn bản không tồn tại. ";
    //                             //     $invalidData = true;

    //                             //     $indexKey = array_search('nguoi_soan_vb_di', $keys) + 1; // bỏ đi cột stt
    //                             //     $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                             //     $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                             //         array(
    //                             //             'fill' => array(
    ...
    //                             if (!$id_don_vi) {
    //                                 $l['ketqua'] .= "Hồ sơ đơn vị không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('don_vi', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                             if (!$id_tinh_chat) {
    //                                 $l['ketqua'] .= "Tính chất không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('ten_tinh_chat', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                             if (!$id_bao_mat) {
    //                                 $l['ketqua'] .= "Bảo mật không đúng. ";
    //                                 $invalidData = true;

    //                                 $indexKey = array_search('ten_bao_mat', $keys) + 1; // bỏ đi cột stt
    //                                 $column = PHPExcel_Cell::stringFromColumnIndex($indexKey);
    //                                 $sheet->getStyle($column . $startRow_columnResult)->applyFromArray(
    //                                     array(
    //                                         'fill' => array(
    ...
    //                                 $countSuccess++;
    //                                 $dataInsert[] = [
    //                                     'so_van_ban' => $l['so_van_ban'],
    //                                     'id_loai' => $id_loai['id_loai'],
    //                                     'trang_thai' => Common::STATUS_VAN_BAN_DI['TAO_MOI']['value'],
    //                                     'loai_van_ban' => 2,
    //                                     'trich_yeu' => $l['trich_yeu'],
    //                                     'nguoi_ky' => $l['nguoi_ky'],
    //                                     'ngay_ky' => $l['ngay_ky'],
    //                                     'tra_loi_cv_den' => $l['tra_loi_cv_den'],
    //                                     'ngay_tra_loi_cv_den' => $l['ngay_tra_loi_cv_den'],
    //                                     'so_hieu_van_ban' => $l['so_hieu_van_ban'],
    //                                     'id_don_vi_soan' => $id_don_vi_soan['id_don_vi'],
    //                                     'id_hinh_thuc' => $id_hinh_thuc['id_hinh_thuc'],
    //                                     // 'nguoi_soan_vb_di' => $nguoi_soan_vb_di['ql_nguoi_dung_id'],
    //                                     'nguoi_soan_vb_di' => $l['nguoi_soan_vb_di'],
    //                                     'linh_vuc' => $l['linh_vuc'],
    //                                     'id_don_vi' => $id_don_vi['id_don_vi'],
    //                                     'noi_luu_tru' => $l['noi_luu_tru'],
    //                                     'id_tinh_chat' => $id_tinh_chat['id_tinh_chat'],
    //                                     'id_bao_mat' => $id_bao_mat['id_bao_mat'],
    //                                 ];
    //                             }
    //                         }
    ...
    public function bao_cao_phan_hoi_create_post($id)
    {
        ...
        $this->createLog('create', 'Báo cáo phản hồi văn bản', [], $baocaophanhoi, 'e_van_ban, e_bao_cao');
        ...
        if ($xuly) {
            ...
            //Tạo thông báo về cho các đơn vị được ban hành
            ...
            if ($donviduocbanhanh) {
                ...
                $noidungthongbao = 'Đơn vị ' . $donvi['ten_don_vi'] . ' đã phản hồi văn bản ' .
                    $vanban['trich_yeu'] . '. Click vào đây để <a href="' . $this->config->item('frontend_url') . 'vanban/vanbandendonvi/xemchitiet/' . $vanban['id_van_ban'] . '">xem chi tiết</a>.';
                $donvithongbaoIds = array_column($donviduocbanhanh, 'id_don_vi');

                $notification = $this->Ql_thong_bao_model->create([
                    'ql_thong_bao_tieu_de' => $noidungthongbao,
                    'ql_thong_bao_tieu_de_tieng_anh' => $noidungthongbao,
    ...
    public function view_log_get()
    {
        ...
        if (isset($this->common)) {
            foreach ($this->common::STATUS_VAN_BAN_DI_DON_VI as $status) {
    ...
    public function search_files_post()
    {
        ...
        $files = $this->E_van_ban_model->getFiles_byLoaiVanBan(5, $data);
