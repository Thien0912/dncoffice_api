...
class Vanbandi extends REST_INSTANCE_Controller
{
    ...
    public function index_get()
    {
        ...
        if (!$auth) {
            resError('Không tìm thấy cái tài khoản', 401);
            return;
        }
        ...
        $data = $this->E_van_ban_model->getAllVanbandi($data['start'], $data['length'], $data['searchValue'], $data['order'], $data['columns'], $data['searchKey'], $data['fromDate'], $data['toDate'], $auth, $dataSource);
    ...
    public function create_post()
    {
        ...
        // Log dữ liệu đầu vào
        $this->createLog('Info', 'Bắt đầu tạo văn bản đi - Dữ liệu đầu vào', NULL, [
            'data' => $data,
            'ids_khoi_co_quan' => $ids_khoi_co_quan,
            'ids_co_quan' => $ids_co_quan,
            'ids_don_vi_xu_ly' => $ids_don_vi_xu_ly,
            'ids_ql_nguoi_dung' => $ids_ql_nguoi_dung,
            'ids_hinh_thuc' => $ids_hinh_thuc,
            'trang_thai_luu_tru' => $trang_thai_luu_tru,
            'is_public' => $is_public,
            'created_by' => $auth['ql_nguoi_dung_id'] ?? null,
            'created_by_name' => $auth['ql_nguoi_dung_ho_ten'] ?? null,
            'created_at' => date('Y-m-d H:i:s')
        ], 'e_van_ban');

        $this->db->db_debug = FALSE; // Tắt để DB error trả JSON thay vì HTML
        $this->db->trans_start();
        ...
        if ($ids_hinh_thuc) {
            if (is_string($ids_hinh_thuc)) {
                $decoded = json_decode($ids_hinh_thuc, true);
                $ids_hinh_thuc = is_array($decoded) ? $decoded : explode(',', $ids_hinh_thuc);
            }
            foreach ($ids_hinh_thuc as $id_ht) {
                if (empty($id_ht))
                    continue;
                $hinhthuc = $this->E_hinh_thuc_model->find($id_ht);

                if ($hinhthuc) {
                    if ($hinhthuc['ma_hinh_thuc'] == 'ZALO')
                        $sendZalo = true;
        ...
        if ($ids_khoi_co_quan) {
            $ids_khoi_co_quan_array = [];
            if (is_string($ids_khoi_co_quan)) {
                $decoded = json_decode($ids_khoi_co_quan, true);
                $ids_khoi_co_quan_array = is_array($decoded) ? $decoded : explode(',', $ids_khoi_co_quan);
            } else if (is_array($ids_khoi_co_quan)) {
                $ids_khoi_co_quan_array = $ids_khoi_co_quan;
            }
            ...
            if (!empty($batch_khoi)) {
                $this->db->insert_batch('e_vb_khoi_co_quan', $batch_khoi);
            }
        }

        if ($ids_co_quan) {
            $ids_co_quan_array = [];
            if (is_string($ids_co_quan)) {
                $decoded = json_decode($ids_co_quan, true);
                $ids_co_quan_array = is_array($decoded) ? $decoded : explode(',', $ids_co_quan);
            } else if (is_array($ids_co_quan)) {
                $ids_co_quan_array = $ids_co_quan;
            }
            ...
            if (!empty($batch_co_quan)) {
                $this->db->insert_batch('e_vb_co_quan', $batch_co_quan);
            }
        }
        ...
        if ($ids_don_vi_xu_ly) {
            ...
            $ids_don_vi_xu_ly_array = [];
            if (is_string($ids_don_vi_xu_ly)) {
                $decoded = json_decode($ids_don_vi_xu_ly, true);
                $ids_don_vi_xu_ly_array = is_array($decoded) ? $decoded : explode(',', $ids_don_vi_xu_ly);
            } else if (is_array($ids_don_vi_xu_ly)) {
                $ids_don_vi_xu_ly_array = $ids_don_vi_xu_ly;
            }
            $don_vi_duoc_chon = $this->E_don_vi_model->whereIn('id_don_vi', $ids_don_vi_xu_ly_array)->get();
            $ids_don_vi_xu_ly_array = array_unique($ids_don_vi_xu_ly_array);


            $tat_ca_don_vi = $this->E_don_vi_model->select('*')->get();
            $ids_don_vi = array_unique(array_column($tat_ca_don_vi, 'id_don_vi'));

            if (empty(array_diff($ids_don_vi, $ids_don_vi_xu_ly_array))) {
                $text_don_vi_xu_ly = 'Tất cả các đơn vị thuộc trường';
            } else {
                foreach ($don_vi_duoc_chon as $key => $dv) {
                    if ($key == (count($don_vi_duoc_chon) - 1)) {
                        $text_don_vi_xu_ly .= $dv['ten_viet_tat'] . '.';
                    } else {
                        $text_don_vi_xu_ly .= $dv['ten_viet_tat'] . ', ';
                    }
                }
            ...
            $vb['ds_don_vi_nhan'] = $text_don_vi_xu_ly;
        }
        ...
        if ($ids_ql_nguoi_dung) {
            ...
            $ids_ql_nguoi_dung_array = [];
            if (is_string($ids_ql_nguoi_dung)) {
                $decoded = json_decode($ids_ql_nguoi_dung, true);
                $ids_ql_nguoi_dung_array = is_array($decoded) ? $decoded : explode(',', $ids_ql_nguoi_dung);
            } else if (is_array($ids_ql_nguoi_dung)) {
                $ids_ql_nguoi_dung_array = $ids_ql_nguoi_dung;
            }
            ...
            if (!empty($ids_ql_nguoi_dung_array)) {
                $all_nguoi_dungs = $this->Ql_nguoi_dung_model->whereIn('ql_nguoi_dung_id', $ids_ql_nguoi_dung_array)->get();
                $current_user_id = $this->getUserLogin()['ql_nguoi_dung_id'];
                $batch_don_vi_xu_ly = [];
                $ngay_tao = date('Y-m-d H:i:s');

                foreach ((array) $all_nguoi_dungs as $nguoidung) {
                    if (!$nguoidung || empty($nguoidung['id_don_vi']))
                        continue;

                    $batch_don_vi_xu_ly[] = [
                        'id_don_vi' => $nguoidung['id_don_vi'],
                        'id_xu_ly' => $xulyId,
                        'nguoi_tao' => $current_user_id,
                        'id_nguoi_xu_ly' => $nguoidung['ql_nguoi_dung_id'],
                        'ngay_tao' => $ngay_tao
                    ];
                }

                if (!empty($batch_don_vi_xu_ly)) {
                    $this->db->insert_batch('e_don_vi_xu_ly', $batch_don_vi_xu_ly);
                }
            }
        }
        ...
        $uploadedFiles = [];
        if (isset($_FILES['file_ban_hanh'])) {
            $files = $_FILES['file_ban_hanh']; // Tên input từ form                        
        ...
        // File nội bộ
        $uploadedFilesTCHC = [];
        if (isset($_FILES['file_noi_bo'])) {
            $files = $_FILES['file_noi_bo']; // Tên input từ form                        
        ...
        if ($trang_thai_luu_tru == 'LUU_TRU') {
            ...
            if ($sendMail) {
                ...
                if ($time_send_mail == 'CUOI_BUOI') {
                    try {
                        ...
                        $this->E_hang_doi_send_mail_model->create($payload_send_mail);

                        // Log success
                        $this->createLog('Success', 'Tạo hàng đợi send mail văn bản đi thành công ID: ' . $vb['id_van_ban'], NULL, [
                            'payload' => $payload_send_mail,
                            'so_hieu_van_ban' => $vb['so_hieu_van_ban'] ?? null,
                            'trich_yeu' => $vb['trich_yeu'] ?? null,
                            'time_send_mail' => $time_send_mail
                        ], 'e_hang_doi_send_mail');

                        $sendMail = false;
                    } catch (Exception $e) {
                        $this->createLog('Error', 'Lỗi khi tạo hàng đợi send mail văn bản đi ID: ' . $vb['id_van_ban'] . ' - ' . $e->getMessage(), NULL, [
                            'id_van_ban' => $vb['id_van_ban'],
                            'loai_van_ban' => 2,
                            'time_send_mail' => $time_send_mail,
                            'sendMail' => $sendMail,
            ...
            try {
                $notification_payload = [
                    ...
                    'so_hieu_van_ban' => $vb['so_hieu_van_ban'] ?? null,
                    'trich_yeu' => $vb['trich_yeu'] ?? null,
                    'ids_don_vi_xu_ly' => $ids_don_vi_xu_ly_array,
                    'ids_ql_nguoi_dung' => $ids_ql_nguoi_dung_array,
                    'sendMail' => false,
                    'sendZalo' => $sendZalo
                ];
                ...
                // Log success
                $this->createLog('Success', 'Tạo thông báo văn bản đi thành công ID: ' . $vb['id_van_ban'], NULL, [
                    'payload' => $notification_payload,
                    'count_don_vi' => count($ids_don_vi_xu_ly_array),
                    'count_nguoi_dung' => count($ids_ql_nguoi_dung_array)
                ], 'ql_thong_bao');
            } catch (Exception $e) {
                $this->createLog('Error', 'Lỗi khi tạo thông báo văn bản đi ID: ' . $vb['id_van_ban'] . ' - ' . $e->getMessage(), NULL, [
                    'id_van_ban' => $vb['id_van_ban'],
                    'so_hieu_van_ban' => $vb['so_hieu_van_ban'] ?? null,
                    'trich_yeu' => $vb['trich_yeu'] ?? null,
                    'ids_don_vi_xu_ly' => $ids_don_vi_xu_ly_array,
                    'ids_ql_nguoi_dung' => $ids_ql_nguoi_dung_array,
                    'sendZalo' => $sendZalo,
                    'auth_id' => $auth['ql_nguoi_dung_id'] ?? null,
                    'error' => $e->getMessage(),
                    'trace' => $e->getTraceAsString()
                ], 'ql_thong_bao');
            }
        ...
        $loaiVB = $this->E_loai_model->find($vb['id_loai']);

        $vb['trich_yeu_tuy_chinh'] = $loaiVB['ten_loai'] . ' số ' . $vb['trich_yeu'];
        $vb['ql_thong_bao_link'] = $this->config->item('frontend_url') . 'vanban/vanbandendonvi/xemchitiet/' . $vb['id_van_ban'];
    ...
    public function show_get($id)
    {
        ...
        if (!$vb)
            resError('Không tìm thấy văn bản', REST_Controller::HTTP_NOT_FOUND);
    ...
    public function checkExitsUpdate($id, $field, $value, $table = 'e_van_ban', $loai_van_ban, $auth)
    {
        ...
        $vbdi = $this->E_van_ban_model
            ...
            ->where('id_don_vi', $tempVB['id_don_vi'])
            ->get();
    ...
    public function update_post($id)
    {
        ...
        if ($ids_hinh_thuc) {
            if (is_string($ids_hinh_thuc)) {
                $decoded = json_decode($ids_hinh_thuc, true);
                $ids_hinh_thuc_array = is_array($decoded) ? $decoded : explode(',', $ids_hinh_thuc);
            } else if (is_array($ids_hinh_thuc)) {
                $ids_hinh_thuc_array = $ids_hinh_thuc;
            }
        }
        $ids_hinh_thuc_old_array = array_column($this->E_vb_hinh_thuc_model->where('id_van_ban', $id)->get(), 'id_hinh_thuc');
        ...
        if (!empty($arrayAdded)) {
            foreach ($arrayAdded as $id_hinh_thuc) {
                if ($id_hinh_thuc && ($this->E_hinh_thuc_model->find($id_hinh_thuc))) {
                    $exist = $this->E_vb_hinh_thuc_model->where('id_van_ban', $id)->where('id_hinh_thuc', $id_hinh_thuc)->get();
                    if (empty($exist)) {
                        $this->E_vb_hinh_thuc_model->create([
                            'id_van_ban' => $id,
        ...
        $ids_khoi_co_quan_old_array = array_column($this->E_vb_khoi_co_quan_model->where('id_van_ban', $id)->get(), 'id_khoi_co_quan');
        ...
        $ids_co_quan_old_array = array_column($this->E_vb_co_quan_model->where('id_van_ban', $id)->get(), 'id_co_quan');
        ...
        $ids_don_vi_xu_ly_old_array = array_column($this->E_don_vi_xu_ly_model->where('id_xu_ly', $xuly['id_xu_ly'])->get(), 'id_don_vi');
        ...
        $ids_ql_nguoi_dung_old_array = array_column($this->E_don_vi_xu_ly_model->where('id_xu_ly', $xuly['id_xu_ly'])->get(), 'id_nguoi_xu_ly');
        ...
        // Xử lý file đính kèm cũ (file_ban_hanh_old)
        $currentFiles = $this->E_file_dinh_kem_model
            ->where('id_van_ban', $id)
            ->where('la_file_ban_hanh', 1)
            ->get();
        // Lấy file cũ CÒN LẠI (ĐƯỢC GIỮ) đã nhận được
        $file_ban_hanh_old = json_decode(commonRequest('file_ban_hanh_old'), true) ?? [];
        $file_ban_hanh_old = array_map(function ($f_old) {
            $f_old['duong_dan'] = decryptString($f_old['duong_dan']);
            return $f_old;
        }, $file_ban_hanh_old); // Mã hóa về đường dẫn gốc
        $fileOldName = array_column($file_ban_hanh_old, 'ten_file_goc');

        foreach ($currentFiles as $key => $file) {
            if (!in_array($file['duong_dan'], array_column($file_ban_hanh_old, 'duong_dan'))) {
        ...
        $currentFiles = $this->E_file_dinh_kem_model
            ...
            ->get();
        // Lấy file cũ CÒN LẠI (ĐƯỢC GIỮ) đã nhận được
        $file_noi_bo_old = json_decode(commonRequest('file_noi_bo_old'), true) ?? [];
        $file_noi_bo_old = array_map(function ($f_old) {
            $f_old['duong_dan'] = decryptString($f_old['duong_dan']);
            return $f_old;
        }, $file_noi_bo_old); // Mã hóa về đường dẫn gốc
        $fileOldName = array_column($file_noi_bo_old, 'ten_file_goc');

        foreach ($currentFiles as $key => $file) {
            if (!in_array($file['duong_dan'], array_column($file_noi_bo_old, 'duong_dan'))) {
                @unlink(FCPATH . $file['duong_dan']);
                $this->E_file_dinh_kem_model->where('id_file_dinh_kem', $file['id_file_dinh_kem'])->delete();
            }
        }
        ...
        //Ghi log
        $this->createLog('update', 'Cập nhật văn bản đi: ' . $vb['so_hieu_van_ban'], $giatricu, $newVb, 'e_van_ban');
    ...
    public function delete_post()
    {
        ...
        $this->createLog('delete', 'Xóa văn bản', $vanban, null, 'e_van_ban');
        $this->db->trans_commit();
        resSuccess(null, 'Đã chuyển vào thùng rác');
    }
    ...
    public function revoke_post()
    {
        $ids = commonRequest('ids');
        $this->db->trans_start();
        $vanban = $this->E_van_ban_model->where('loai_van_ban', 2)->whereIn('id_van_ban', $ids)->where('deleted_at IS NULL')->get();
        if (count($ids) != count($vanban)) {
            resError('Có văn bản không tồn tại');
        }
    ...
    public function export_get()
    {
        ...
        $data = $this->E_van_ban_model->getListExportVanbandi($start, $length, $searchValue, $searchKey, $fromDate, $toDate, $auth);
            ...
            foreach ($titles as $key => $title) {
                ...
                if ($key == 'noi_nhan') {
                    ...
                    $e_don_vi_xu_ly = $this->E_xu_ly_model
                        ->distinct()
                        ->select('e_don_vi.id_don_vi, e_don_vi.ten_don_vi, e_don_vi.ten_viet_tat')
                        ->leftJoin('e_don_vi_xu_ly', 'e_don_vi_xu_ly.id_xu_ly = e_xu_ly.id_xu_ly')
                        ->leftJoin('e_don_vi', 'e_don_vi.id_don_vi = e_don_vi_xu_ly.id_don_vi')
                    ...
                    if ($e_don_vi_xu_ly) {
                        foreach ($e_don_vi_xu_ly as $key => $itemdvxl) {
                            $text_noi_nhan .= $itemdvxl['ten_viet_tat'] . ', ';
                        }
                    }
        ...
        try {
            ...
            if ($file_import) {
                ...
                if (!$this->upload->do_upload('file_excel')) {
                    ...
                    // START CUSTOM: Prepare Excel file data for attachment
                    $excelAttachmentData = [
                        'file_name' => $fileName,
                        'file_size' => $uploadData['file_size'],
                        'file_path' => $path, // This path is relative to BASEPATH/../ or absolute? CI upload returns full path usually? No, returns info.
                        // CI upload data 'full_path' is absolute. 'file_path' is directory.
                        // We need the relative path for viewing later possibly, or we just store 'uploads/excel/vanbandi/filename'
                        'relative_path' => 'uploads/excel/vanbandi/' . $fileName,
                        'file_extension' => str_replace('.', '', $uploadData['file_ext'])
                    ];
                    // END CUSTOM
                    ...
                    if (!empty($dataList)) {
                        ...
                        // START CUSTOM: Upload files like create_post
                        $is_public = commonRequest('is_public');
                        $folderName = 'documents/' . date('Y') . '/' . date('m');

                        $uploadedFiles_BanHanh = [];
                        if (isset($_FILES['file_ban_hanh'])) {
                            $files = $_FILES['file_ban_hanh'];
                            foreach ($files['name'] as $key => $fileName) {
                                $file = [
                                    'name' => $files['name'][$key],
                                    'type' => $files['type'][$key],
                        ...
                        foreach ($dataList as $key => $l) {
                            ...
                            if (!empty($errorMessages)) {
                                ...
                                if (!$invalidData) {
                                    ...
                                    $hauToVanBan = $l['so_hieu_van_ban'];
                                    $hauToVanBan = explode('/', $hauToVanBan);
                                    $hauToVanBan = end($hauToVanBan);

                                    $LoaiVanBan = $this->db->select('*')
                                        ->from('e_loai')
                                        ->where('hau_to', $hauToVanBan)
                                        ->get()
                                        ->row();
                                    ...
                                    $dataInsert = [
                                        ...
                                        'id_loai' => $LoaiVanBan ? $LoaiVanBan->id_loai : null,
                                    ...
                                    // START CUSTOM: Attach files like create_post
                                    foreach ($uploadedFiles_BanHanh as $uploadedFile) {
                                        $this->E_file_dinh_kem_model->create([
                                            'id_van_ban' => $id_van_ban,
                                            'ten_file_goc' => $uploadedFile['file_name'],
                                            'dung_luong' => exchangeFromKbToLargerCapacity($uploadedFile['file_size']),
                                            'duong_dan' => $uploadedFile['file_path'],
                                            'loai_file' => $uploadedFile['file_extension'],
                                            'la_file_ban_hanh' => 1,
                                            'is_public' => $is_public
                                    ...
                                    // Attribute Excel File to this VanBan
                                    if (!empty($excelAttachmentData)) {
                                        $this->E_file_dinh_kem_model->create([
                                            'id_van_ban' => $id_van_ban,
                                            'ten_file_goc' => $excelAttachmentData['file_name'],
                                            'dung_luong' => exchangeFromKbToLargerCapacity($excelAttachmentData['file_size']),
                                            'duong_dan' => $excelAttachmentData['relative_path'],
                                            'loai_file' => $excelAttachmentData['file_extension'],
                                            'la_file_ban_hanh' => 0, // Is it noi bo or ban hanh? Let's say 0 (internal)
                                            'is_public' => $is_public
                                        ]);
                                    }

                                    foreach ($uploadedFiles_NoiBo as $item) {
                                        $this->E_file_dinh_kem_model->create([
                                            'id_van_ban' => $id_van_ban,
                                            'ten_file_goc' => $item['file_name'],
                                            'dung_luong' => exchangeFromKbToLargerCapacity($item['file_size']),
                                            'duong_dan' => $item['file_path'],
                                            'loai_file' => $item['file_extension'],
                                            'la_file_ban_hanh' => 0,
                                            'is_public' => $is_public
                ...
                if ($stop) {
                    if (isset($unlinkFile) && $unlinkFile)
                        unlink($path);
                    ...
                    // Xóa file -> Disable delete if we attached it
    ...
    public function view_log_get()
    {
        ...
        if (!$id_van_ban) {
            resError('Thiếu id_van_ban', REST_INSTANCE_Controller::HTTP_BAD_REQUEST);
        }

        // Status mapping
        $statusMap = [];
        if (isset($this->common)) {
            foreach ($this->common::STATUS_VAN_BAN_DI as $status) {
    ...
    public function files_v2_post()
    {
        ...
        $files = $this->E_van_ban_model->getFiles_v2(2, $offset, $limit, $filters);
    ...
    public function cloneDocument_get($id_van_ban)
    {
        ...
        $vanBanNhanBan['trich_yeu'] = $trich_yeu_moi;
