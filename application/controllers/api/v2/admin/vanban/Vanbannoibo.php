...
class Vanbannoibo extends REST_INSTANCE_Controller
{
    ...
    public function index_get()
    {
        ...
        $data = $this->E_van_ban_model->getAllVanbannoibo($data['start'], $data['length'], $data['searchValue'], $data['order'], $data['columns'], $data['searchKey'], $data['fromDate'], $data['toDate'], $auth, $dataSource);
    ...
    public function show_get($id)
    {
        ...
        if (!$isSuperAdmin) {
            $this->E_van_ban_model->where('e_don_vi.id_don_vi', $auth['id_don_vi']);
        }

        $vb = $this->E_van_ban_model->first();

        if (!$vb) resError('Không tìm thấy văn bản', REST_Controller::HTTP_NOT_FOUND);

        $file_noi_bo = $this->E_file_dinh_kem_model->where('id_van_ban', $id)->get();
        foreach ($file_noi_bo as &$f) {
            $f['duong_dan'] = encryptString($f['duong_dan']);
        }
        unset($f);

        $vb['files'] = $file_noi_bo;
        ...
        $vb['e_vb_co_quan'] = $this->E_vb_co_quan_model
            ...
            ->where('id_van_ban', $id)
            ->get();
        $vb['nguoi_xem'] = $this->db
            ->select('e_nguoi_xem_vbnoibo.*, ql_nguoi_dung.ql_nguoi_dung_ho_ten')
            ->from('e_nguoi_xem_vbnoibo')
            ->join('ql_nguoi_dung', 'ql_nguoi_dung.ql_nguoi_dung_id = e_nguoi_xem_vbnoibo.ql_nguoi_dung_id', 'left')
            ->where('id_van_ban', $id)
            ->get()
            ->result_array();
    ...
    public function checkExitsUpdate($id, $field, $value, $table = 'e_van_ban', $loai = 3)
    {
        $auth = $this->getUserLogin();
    ...
    public function move_to_trash_post()
    {
        ...
        $this->createLog('move_to_trash', 'Chuyển văn bản nội bộ vào thùng rác', $vanban, null, 'e_van_ban');
        $this->db->trans_commit();
    ...
    public function restore_post()
    {
        ...
        $user = $this->getUserLogin();
        $this->db->where_in('id_van_ban', $ids)
            ->where('id_don_vi', $user['id_don_vi'])
            ->delete('e_van_ban_da_xoa');

        $this->createLog('restore', 'Khôi phục văn bản nội bộ', $ids,  null, 'e_van_ban');
        $this->db->trans_commit();
    ...
    public function update_files_post($id)
    {
        ...
        try {
            ...
            //Ghi log
            $this->createLog('update', 'Cập nhật file', $currentFiles, $dataNew, 'e_van_ban');
            $this->db->trans_commit();
    ...
    public function files_v2_post()
    {
        ...
        $files = $this->E_van_ban_model->getFiles_v2(3, $offset, $limit, $filters);
    ...
    public function view_log_get()
    {
        ...
        if (isset($this->common)) {
            foreach ($this->common::STATUS_VAN_BAN_NOI_BO as $status) {
    ...
    public function cloneDocument_get($id_van_ban)
    {
        ...
        unset($vanBanNhanBan['ngay_tao']);
        unset($vanBanNhanBan['ngay_sua']);

        $trich_yeu_moi = "(Bản sao ngày " . date('d/m/Y H:i:s') . ")\n" . $vanBanGoc['trich_yeu'];
        $vanBanNhanBan['trich_yeu'] = $trich_yeu_moi;
        $vanBanNhanBan['nam'] = date('Y');

        $user = $this->getUserLogin();
        $vanBanNhanBan['id_nguoi_tao'] = $user['ql_nguoi_dung_id'];
        $vanBanNhanBan['id_nguoi_sua'] = $user['ql_nguoi_dung_id'];
        
        // Cập nhật đơn vị là đơn vị của người dùng thao tác clone
        $vanBanNhanBan['id_don_vi'] = $user['id_don_vi'];
        $vanBanNhanBan['id_don_vi_soan'] = $user['id_don_vi'];

        $this->db->trans_start();

        // 1. Tao ban ghi moi
        $newVb = $this->E_van_ban_model->create($vanBanNhanBan);
        $newId = $newVb['id_van_ban'];

        // 2. Clone gioi han doc (e_nguoi_xem_vbnoibo)
        $nguoiXems = $this->db->where('id_van_ban', $id_van_ban)
                              ->get('e_nguoi_xem_vbnoibo')->result_array();
                              
        foreach($nguoiXems as $nx) {
            $dataNx = $nx;
            unset($dataNx['id']); // Xóa ID tự tăng
            $dataNx['id_van_ban'] = $newId;
            $this->db->insert('e_nguoi_xem_vbnoibo', $dataNx);
        }

        // 3. Clone khoi co quan
        $this->load->model('E_vb_khoi_co_quan_model');
        $khoiCoQuans = $this->E_vb_khoi_co_quan_model->where('id_van_ban', $id_van_ban)->get();
        foreach($khoiCoQuans as $kcq) {
            $dataKcq = $kcq;
            unset($dataKcq['id_vb_khoi_co_quan']);
            $dataKcq['id_van_ban'] = $newId;
            $this->E_vb_khoi_co_quan_model->create($dataKcq);
        }

        // 4. Clone co quan
        $this->load->model('E_vb_co_quan_model');
        $coQuans = $this->E_vb_co_quan_model->where('id_van_ban', $id_van_ban)->get();
        foreach($coQuans as $cq) {
            $dataCq = $cq;
            unset($dataCq['id_vb_co_quan']);
            $dataCq['id_van_ban'] = $newId;
            $this->E_vb_co_quan_model->create($dataCq);
        }

        // 5. Clone files
        $files = $this->E_file_dinh_kem_model->where('id_van_ban', $id_van_ban)->get();
        ...
        $this->createLog('Create', 'Tạo bản sao văn bản nội bộ', NULL, $newVb, 'e_van_ban');
        
        $this->db->trans_commit();

        call_n8n_trich_xuat($newId);

        resSuccess($newVb, 'Tạo bản sao thành công');
    }
}
