...
class Daotao extends REST_INSTANCE_Controller
{
    ...
    public function create_nhanvien_daotao_post()
    {
        ...
        $khoaDaoTao = $this->Hrm_dao_tao_model->find($id_dao_tao);
        ...
        $changes = [
            [
                'field' => 'dao_tao',
                'field_name' => 'Quá trình đào tạo',
                'old_value' => null,
                'new_value' => [
                    'value' => 'Thêm mới',
                    'label' => 'Khóa học: ' . ($khoaDaoTao['ten_khoa_hoc'] ?? '') . ' - Kết quả: ' . $ket_qua
                ]
            ]
        ];
        foreach ($ids_nhan_vien as $id) {
            $this->logEmployeeHistory($id, 'Thêm nhân viên vào khóa đào tạo', $changes);
        }
    ...
    public function edit_nhanvien_daotao_post($id)
    {
        ...
        $this->Hrm_nhan_vien_dao_tao_model
            ->where('id_nhan_vien_dao_tao', $id)
            ->update($data);

        $nhanvien_daotao['data'] = $this->Hrm_nhan_vien_dao_tao_model->find($id);
        ...
        $oldDaoTao = $this->Hrm_dao_tao_model->find($oldData['id_dao_tao']);
        $newDaoTao = $this->Hrm_dao_tao_model->find($data['id_dao_tao']);
        ...
        $changes = [
            [
                ...
                'old_value' => [
                    'value' => 'Cập nhật',
                    'label' => 'Khóa học: ' . ($oldDaoTao['ten_khoa_hoc'] ?? '') . ' - Kết quả: ' . $oldData['ket_qua']
                ],
                'new_value' => [
                    'value' => 'Cập nhật',
                    'label' => 'Khóa học: ' . ($newDaoTao['ten_khoa_hoc'] ?? '') . ' - Kết quả: ' . $data['ket_qua']
                ]
            ]
        ];
        $this->logEmployeeHistory($oldData['id_nhan_vien'], 'Cập nhật quá trình đào tạo', $changes);
        
        $this->db->trans_commit();
        resSuccess($nhanvien_daotao, 'Sửa thành công', REST_INSTANCE_Controller::HTTP_CREATED);
    }

    public function delete_nhanvien_daotao_post()
    {
        $ids = commonRequest('ids');
        $oldData = $this->Hrm_nhan_vien_dao_tao_model
            ->select('hrm_nhan_vien_dao_tao.*, hrm_dao_tao.ten_khoa_hoc')
            ->join('hrm_dao_tao', 'hrm_dao_tao.id_dao_tao = hrm_nhan_vien_dao_tao.id_dao_tao', 'left')
            ->whereIn('id_nhan_vien_dao_tao', $ids)->get();
        ...
        if ($id_nhan_vien) {
            foreach($oldData as $item) {
                $changes[] = [
                    ...
                    'old_value' => [
                        'value' => 'Xoá',
                        'label' => 'Khóa học: ' . ($item['ten_khoa_hoc'] ?? '') . ' - Kết quả: ' . $item['ket_qua']
                    ],
                    'new_value' => null
                ];
            }
    ...
    public function update_post($id)
    {
        ...
        $khoadaotao = $this->Hrm_dao_tao_model
            ->where('id_dao_tao', $id)
            ->update($data);

        //create log
        $this->createLog('update', 'Cập nhật khóa đào tạo', null, $khoadaotao, 'hrm_dao_tao');
        $this->db->trans_commit();
        resSuccess($khoadaotao, 'Cập nhật thành công', REST_INSTANCE_Controller::HTTP_CREATED);
    }
}
