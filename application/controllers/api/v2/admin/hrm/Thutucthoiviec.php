...
class Thutucthoiviec extends REST_INSTANCE_Controller
{
    ...
    public function delete_post($id)
    {
        ...
        if (!empty($nhanVienThoiViec)) {
            resError('Thủ tục này đang được sử dụng!');
        }
    ...
    public function deletes_post()
    {
        ...
        if (empty($ids)) {
            resBadrequest(null, 'Vui lòng chọn thủ tục cần xóa');
        }
        ...
        foreach ($ids as $id) {
            ...
            // Kiểm tra xem thủ tục có đang được sử dụng không
            $nhanVienThoiViec = $this->db
                ->from('hrm_nhan_vien_thoi_viec')
                ->select('*')
                ->where('id_tttv', $id)
        ...
        if ($deletedCount === 0 && $failedCount > 0) {
            resError('Các thủ tục đã chọn đang được sử dụng, không thể xóa!');
        }

        $message = "Đã xóa thành công $deletedCount thủ tục.";
        if ($failedCount > 0) {
            $message .= " Có $failedCount thủ tục đang được sử dụng không thể xóa.";
        }

        resSuccess(null, $message);
    }
