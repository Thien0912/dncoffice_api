...
class Fileaccess extends CI_Controller
{
    ...
    public function imageFromEncryptedPath($link)
    {
        $decryptLink = decryptString($link);
        $arr = explode('?', $decryptLink); //Tách chuỗi
        $arr[0] = decryptString($arr[0]); // mã hóa đường dẫn về đường dẫn gốc
        if (empty($arr[0])) {
    ...
    public function forceDownloadFile($link = NULL)
    {
        if (!$link) show_404();
        $path = $this->decryptUrl($link);
        $queryParams = [];
        if (isset($path[1])) {
            parse_str($path[1], $queryParams);
        }
        $realPath = FCPATH . ltrim($path[0], '/');
        $downloadName = $queryParams['fileName'] ?? basename($realPath);
    ...
    {
        if (empty($arr)) die('Lỗi khi xử lý thông tin đăng nhập!');
        $user_id = $arr['user_id'];
        $user_seen = $arr['user_seen'];
        ...
        if ($file) {
            if (!in_array($file['is_public'], [self::FILE_PRIVATE, self::FILE_INTERNAL, self::FILE_PUBLIC])) die('The public status of the file is invalid');
        ...
        switch ($is_public) {
                        ...
                        if ($ql_nguoi_dung['ql_nguoi_dung_is_admin'] != 1) {
                            ...
                            switch ($model_tuong_ung) {
                                case 'van_ban':
                                    // Tìm văn bản
                                    $van_ban = $this->E_van_ban_model->find($file['id_van_ban']);
                                    if (!$van_ban) $this->checkViewAccessAndDie('INTERNAL__Không tìm thấy văn bản!');
                                    $nguoi_tao =  $this->Ql_nguoi_dung_model->find($van_ban['id_nguoi_tao'])['id_don_vi'];

                                    if (in_array($van_ban['trang_thai'], [
                                        ...
                                        if (
                                            ...
                                            if (!$xu_ly) die('INTERNAL__Không tìm thấy xử lý!');

                                            // Tìm đơn vị xử lý
                                            $donvixuly = $this->E_don_vi_xu_ly_model->where('id_xu_ly', $xu_ly)->get();
                                            if (!$donvixuly) $this->checkViewAccessAndDie('INTERNAL__Không tìm thấy đơn vị xử lý!');
                                ...
                                case 'hop_dong':
                                    ...
                                    if ($donvi['ma_don_vi'] != 'PHONG_TCHC') $this->checkViewAccessAndDie('PRIVATE__Không có quyền của đơn vị TC-HC!');
                                    break;
            ...
            case self::NOT_A_DOCUMENT:
                ...
                if (!$user_id && !$user_seen) {
                    ...
                    $ql_nguoi_dung = $this->Ql_nguoi_dung_model->find($user_seen);
                    if (!$ql_nguoi_dung) die('NOT_A_DOCUMENT__File chỉ được phép truy cập bởi người dùng đã được đăng ký!');
                }
                break;
            default:
                // File chỉ người tạo văn bản hoặc người cùng đơn vị được thấy
                if (!$user_id && !$user_seen) {
                    ...
                    if (!$ql_nguoi_dung) die('default__File chỉ được phép truy cập bởi người dùng đã được đăng ký!');

                    if ($ql_nguoi_dung['ql_nguoi_dung_is_admin'] != 1) {
                        $id_don_vi = $ql_nguoi_dung['id_don_vi'];

                        if (!$id_don_vi) die('default__Không tìm thấy thông tin đơn vị!');

                        // Tìm văn bản
                        $van_ban = $this->E_van_ban_model->find($file['id_van_ban']);
                        if (!$van_ban) die('default__Không tìm thấy văn bản!');

                        $nguoi_tao = $this->Ql_nguoi_dung_model->find($van_ban['id_nguoi_tao']);
                        if (!$nguoi_tao) die('default__Không tìm thấy thông tin người tạo!');

                        if ($nguoi_tao['id_don_vi'] != $id_don_vi) die('default__Không có quyền truy cập file!');
                    }
                }
                break;
        }
