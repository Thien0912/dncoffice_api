...
class Hopdong extends REST_INSTANCE_Controller
{
    ...
    public function index_get()
    {
        ...
        $orderPayload = commonRequest('order') ?? [];
        if (!empty($orderPayload)) {
            foreach ($orderPayload as $key => &$item) {
                $item['column'] = $columns[$item['column']]['data'];
            }
        }
        ...
        // ] : [];
        // $searchKey = commonRequest('searchKey') ? commonRequest('searchKey') : [];
        // $data = $this->Hrm_hop_dong_model->getAll($start, $length, $searchValue, $orderBy, $searchKey, $fromDate, $toDate);
    ...
    public function create_post()
    {
        ...
        $hopdong['luong_co_ban'] = $luongCanBan;
    ...
    public function xemphuluc_get($id)
    {
        ...
        // if ($dsPhuLuc) {
        //     foreach ($dsPhuLuc as &$item) {
        //         $item['ds_phu_cap'] = [];

        //         if ($item['ids_phu_cap']) {
        //             $ids_phu_cap = explode(',', $item['ids_phu_cap']);

        //             $dsPhuCap = $this->Hrm_phu_cap_model
        //                 ->select('*')
        //                 ->whereIn('id_phu_cap', $ids_phu_cap)
        //                 ->get();

        //             $item['ds_phu_cap'] = $dsPhuCap;
        //         }
        //     }
        // }

        // $data['ds_phu_luc'] = $dsPhuLuc;
        // $data['tong_phu_luc'] = count($dsPhuLuc);
    ...
    public function view_log_get()
    {
        $log = $this->Ql_nhat_ky_model
            ->select("
                ...
                JSON_UNQUOTE(JSON_EXTRACT(ql_nhat_ky_gia_tri_moi, '$.id_hop_dong')) AS id_hop_dong
            ")
