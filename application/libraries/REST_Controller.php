...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        // checking for token
        if ($this->config->item('rest_jwt_token')) {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        // Custom Zalo Login
        if (($controller_method == 'loginZaloCallbackReact_get') || ($controller_method == 'loginZaloCallbackReact_post')) {
            // bỏ qua
        } else 
    ...
    {
        $jwt_key = $this->config->item('rest_jwt_token_key');
    ...
    {
        $jwt = new JWT();
        $JwtSecrectKey = $this->config->item('rest_jwt_token_key');
    ...
    {
        // 1. Ưu tiên lấy từ sso-authorization (V2)
        $token = $this->input->get_request_header($this->config->item('rest_token_sso_key'));

        // 2. Nếu không có, lấy theo cấu hình cũ (DHNCT-Authorization)
        if (empty($token)) {
            $token = $this->input->get_request_header($this->config->item('rest_jwt_token_header_key'));
        }
    ...
    public function decodetoken()
    {
        ...
        if ($token) {
            try {
                $decode_token = $this->verifyauthtoken($token);
                if ($decode_token) {
                    $this->saveToken($token, (array)$decode_token);
                    return $this->decode_token = $decode_token;
                }
            } catch (Exception $e) {
            }

            $uSSO = $this->getUserInfo_SSO($token);
            if (strtolower($uSSO['status']) === strtolower('error') || !$uSSO['data']) {
                $this->response([
                    'status' => 401,
                    'message' => "Token SSO không hợp lệ hoặc đã hết hạn",
                    'success' => false
            ...
            if (!$email) {
                $this->response([
                    'status' => 401,
                    'message' => "Email user SSO không tồn tại",
                    'success' => false
                ], REST_Controller::HTTP_UNAUTHORIZED);
            }
            $user = $this->findUserInSystem(['ql_nguoi_dung_email' => $email ?? null]);
            // dd($user);
            if ($user) {
                $this->saveToken($token, (array)$user);
                return $this->decode_token = (object)$user;
            } else {
                // Tạo mới người dùng
                // chờ phân quyền sau
                $dataNguoidung = [
                    'ql_nguoi_dung_ho_ten' => $data['name'] ?? 'Người dùng SSO',
                ...
                if ($nguoidungId) {
                    $user = $this->findUserInSystem(['ql_nguoi_dung_id' => $nguoidungId]);
                    if ($user) {
                        $this->saveToken($token, (array)$user);
                        return $this->decode_token = (object)$user;
                    }
    ...
    public function decodetoken_old()
    {
        $token = $this->input->get_request_header($this->config->item('rest_jwt_token_header_key'));
        try {
            $decode_token = $this->verifyauthtoken($token);
    ...
    public function checkPemissions($permission)
    {
        $groupID = $this->decode_token !=  FALSE ?  $this->decode_token->nhom_phan_quyen : 0;
        ...
        if (!$this->permissions) {
            $error = [
                "status" => FALSE,
                'message' => "Bạn không có quyền nào ! liên hệ quản trị viên để được cấp quyền !",
                'success' => false
            ];
        ...
        if (!in_array($permission, $this->permissions)) {
            $error = [
                "status" => FALSE,
                'message' => "Bạn không có quyền truy cập ! Vui lòng liên hệ quản trị viên để được cấp quyền !",
                'success' => false
            ];
            $this->response($error, REST_Controller::HTTP_FORBIDDEN);
            return FALSE;
    ...
    public function getCon()
    {
        ...
        if ($params) :
            if (array_key_exists("id", $params)) {
                $this->con = array('id' => $params['id']);
                $this->con_total = array('id' => $params['id']);
            } else {
                foreach ($params as $key => $value) {
                    if ($key === 's') {
                        $this->con['s'] = $value;
                        $this->con_total['s'] = $value;
                    } else
                if ($key === 'start') {
                        $this->con['start'] = $value;
                    } else
                if ($key === 'limit') {
                        $this->con['limit'] = $value;
                    } else {
                        $this->con['conditions'][$key] = $value;
                        $this->con_total['conditions'][$key] = $value;
                    }
                }
            }
        endif;
    ...
    // Lấy thông tin người dùng từ SSO bằng token
    public function getUserInfo_SSO($sso_token = null)
    {
        // Nếu không truyền token thì lấy từ header cấu hình của bạn
        $sso_token = $sso_token ?? $this->getAuthTokenFromHeader();

        $url_sso = rtrim($this->config->item('sso_url'), '/') . '/oauth2/userinfo';
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url_sso);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
        curl_setopt($ch, CURLOPT_TIMEOUT, 10);
        // Nếu môi trường dev/sandbox cần bỏ verify (chỉ dùng trong dev):
    ...
    // Kiểm tra tài khoản trong hệ thống bằng email từ sso để lấy thông tin người dùng
    public function findUserInSystem($conditions = [], $google_picture = null)
    {
        $this->load->model('Ql_nguoi_dung_model');
        $this->load->model('Hrm_nhan_vien_model');
        $query = $this->Ql_nguoi_dung_model
            ...
            ->join('e_lanh_dao_don_vi', 'e_lanh_dao_don_vi.ql_nguoi_dung_id = ql_nguoi_dung.ql_nguoi_dung_id', 'left')
            ->select("
                    ql_nguoi_dung.*, 
                    e_don_vi.ten_don_vi, 
                    e_don_vi.ma_don_vi, 
                    ...
                    hrm_nhan_vien.avatar AS hrm_avatar,
                    (SELECT 
                        CASE 
                            WHEN is_valid = 1 THEN 1 
                            WHEN is_valid = 0 AND send_mail_at IS NOT NULL AND pin_failed_attempts = 0 THEN 2 
                            WHEN is_valid = 0 AND send_mail_at IS NOT NULL AND pin_failed_attempts >= 5 THEN 3
                            ELSE NULL 
                        END 
                    FROM ql_nguoi_dung_pin_code 
                    WHERE ql_nguoi_dung_id = ql_nguoi_dung.ql_nguoi_dung_id 
                      ORDER BY created_at DESC
                    LIMIT 1) as has_pin,
                    (SELECT id_ql_nguoi_dung_pin_code 
                    FROM ql_nguoi_dung_pin_code 
                    WHERE ql_nguoi_dung_id = ql_nguoi_dung.ql_nguoi_dung_id 
                    ORDER BY created_at DESC
                    LIMIT 1) as pin_code_id,
                    
                    CASE 
                        WHEN e_lanh_dao_don_vi.id_lanh_dao_don_vi IS NOT NULL 
                            AND e_don_vi.ma_don_vi = 'PHONG_TCHC'
                            THEN 'LANH_DAO_TCHC'
                        WHEN e_lanh_dao_don_vi.id_lanh_dao_don_vi IS NOT NULL
                            THEN 'LANH_DAO_DON_VI'
                        ELSE NULL
                    END AS loai_lanh_dao
                ");

        foreach ($conditions as $field => $value) {
            $query->where($field, $value);
        }
        $query->where('ql_nguoi_dung.active_flag', 1);
        $user = $query->first();

        if ($user) {
            $user = (array)$user;

            // Lấy danh sách vai trò đầy đủ thông tin
            $vaiTros = $this->db
                ->select('vtn.ql_nguoi_dung_id , vtn.ql_vai_tro_id, vtn.is_active, vt.ql_ma_vai_tro, vt.ql_vai_tro_ten')
                ->from('ql_vai_tro_nguoi_dung vtn')
                ->join('ql_vai_tro vt', 'vt.ql_vai_tro_id = vtn.ql_vai_tro_id', 'left')
                ->where('vtn.ql_nguoi_dung_id', $user['ql_nguoi_dung_id'])
                ->get()
                ->result_array();

            $user['vai_tro'] = $vaiTros;

            // Xử lý cập nhật avatar từ Google nếu cả ql_nguoi_dung_avatar và hrm_avatar đều đang trống
            if (empty($user['ql_nguoi_dung_avatar']) && empty($user['hrm_avatar']) && !empty($google_picture)) {
                $this->Ql_nguoi_dung_model->where('ql_nguoi_dung_id', $user['ql_nguoi_dung_id'])->update([
                    'ql_nguoi_dung_avatar' => $google_picture
                ]);
                $this->Hrm_nhan_vien_model->where('ql_nguoi_dung_id', $user['ql_nguoi_dung_id'])->update([
                    'avatar' => $google_picture
                ]);
                $user['ql_nguoi_dung_avatar'] = $google_picture;
            }

            // Ưu tiên lấy avatar từ hrm_nhan_vien, nếu không có mới lấy ql_nguoi_dung_avatar
            if (!empty($user['hrm_avatar'])) {
                $user['ql_nguoi_dung_avatar'] = $user['hrm_avatar'];
            }

            // Mã hóa avatar nếu có và không phải URL
            if (!empty($user['ql_nguoi_dung_avatar']) && !filter_var($user['ql_nguoi_dung_avatar'], FILTER_VALIDATE_URL)) {
                try {
                    ...
                    // Ignore if encrypt failed or helper not found
                }
            }

            unset($user['ql_nguoi_dung_token']);
    ...
    {
        // Ưu tiên token truyền vào, nếu không có mới lấy từ header
        $token = $token ? $token : $this->getAuthTokenFromHeader();
        ...
        // Kiểm tra xem token đã tồn tại chưa để tránh ghi đè/xóa gây race condition
        $existing = $this->Ql_personal_access_token_model
        ...
        // Nếu token chưa có, xóa các token cũ của user này (nếu muốn duy trì 1 phiên đăng nhập)
        // Lưu ý: Nếu muốn cho phép đa thiết bị, hãy bỏ dòng delete này.
        $this->Ql_personal_access_token_model->where('ql_nguoi_dung_id', $user['ql_nguoi_dung_id'])->delete();

        $sql = "INSERT INTO ql_personal_access_token (token, ql_nguoi_dung_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE ql_nguoi_dung_id = VALUES(ql_nguoi_dung_id)";
        $this->db->query($sql, [$md5Token, $user['ql_nguoi_dung_id']]);

        // // Cập nhật token chưa mã hóa vào bảng ql_nguoi_dung
        $this->load->model('Ql_nguoi_dung_model');
        $this->Ql_nguoi_dung_model
            ->where('ql_nguoi_dung_id', $user['ql_nguoi_dung_id'])
            ->update(['ql_nguoi_dung_token' => $token]);
